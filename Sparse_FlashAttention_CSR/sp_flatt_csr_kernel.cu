...
// The sparse-FlashAttention (CSR mask) forward CUDA kernel.
template <typename scalar_t>
__global__ void spfa_csr_cuda_forward_kernel(
    ...
    const torch::PackedTensorAccessor64<unsigned long int, 1, torch::RestrictPtrTraits> W_row_off,
  ...
  int lowInd = W_row_off[blockIdx.x];
  int upInd = W_row_off[blockIdx.x + 1];
  ...
  for (int b_i = 0; b_i < blockIters; b_i++) {
    ...
    if (threadIdx.x < maskElePerIter) {
      W_col_ind_shared[threadIdx.x] = W_col_ind[lowInd + threadIdx.x];
...
// The CUDA kernel dispatch interface for the sparse-FlashAttention (CSR mask) forward CUDA kernel.
torch::Tensor spfa_csr_cuda_forward(
  ...
  // AT_DISPATCH_FLOATING_TYPES(Q.type(), "spfa_csr_forward_cuda", ([&] {
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.type(), "spfa_csr_forward_cuda", ([&] {
    spfa_csr_cuda_forward_kernel<scalar_t><<<blocks, threads, maxSMemPB>>>(
        ...
        W_row_off.packed_accessor64<unsigned long int, 1, torch::RestrictPtrTraits>(),
