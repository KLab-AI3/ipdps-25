...
torch::Tensor spfa_global_no_local_forward(
    torch::Tensor Q,
    torch::Tensor K,
    torch::Tensor V,
    torch::Tensor m,
  ...
  CHECK_INPUT(l);
  CHECK_INPUT(O);
  CHECK_INPUT(global_inds);

  // Return the result of the dispatch function.
  return spfa_global_no_local_cuda_forward(Q, K, V, m, l, O, global_inds, local_size_each_dir);
}


// Binding function for Python.
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &spfa_global_no_local_forward, "Sparse-FlashAttention forward (CUDA)");
}